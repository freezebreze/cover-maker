...
interface RightPropertyPanelProps {
  onImageDowloadClick: (type: imgType) => void;
  onPropInfoChange: (p: PropertyInfo) => void;
}
export const RightPropertyPanel = React.memo(
  (props: RightPropertyPanelProps) => {
    ...
    const [logoPosition, setLogoPosition] = React.useState<string | number>(
      config.logoPosition
    );
    const t = useTranslations("default.RightPropertyPanel");
    ...
    React.useEffect(() => {
      var icon = "";
      if ((deviconValue as Set<string | number>).size > 0) {
